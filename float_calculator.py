...
def float_to_binary(num):
    """Converts the value of a float input to a 32 or 64 bit 
    binary representation returned as a string."""
    EXP_BIAS = 127
...
def int_to_binary(int_num):
    """Converts value of an integer to a 32 or 64 bit binary"""

    if (int_num < 0) or (int_num > 0 and int_num < 1) or (type(int_num) == float):
        return "Invalid"
    elif int_num == 0:
        return "0"    
